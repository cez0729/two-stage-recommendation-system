# 两阶段电商推荐系统：从数据到云端的完整项目说明

> 本文件是本项目的“总档案”。它面向第一次阅读项目的 AI、导师、面试官或合作者，
> 解释项目为什么做、数据从哪里来、每一步如何运行、结果如何解释，以及哪些结论
> 可以说、哪些结论不能说。
>
> 文档更新时间：2026-09-01
> 项目根目录：GitHub 仓库根目录
> 当前主数据集：Amazon Reviews 2018 `Video_Games` 5-core
> 当前云端状态：8000 运行 `recsys_baseline_v1`；8001 的 `recsys_phase6_content_v1` 已验收
...
它是一个**研究生申请/实习作品集级别的完整研究原型和 production-style demo**，不是已经拥有真实用户流量的商业生产系统。数据是公开离线评论数据，没有真实曝光日志、线上点击率、转化率、GMV 或 A/B 测试，因此不能宣称“上线后业务指标提升”。
